...
import { nextPaidPlan, noteLimitForPlan } from "@/lib/usage-limits";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const noteLimit = noteLimitForPlan(org?.subscription_tier);
    const rl = rateLimit(rlKey(user.id, "generate-note"), noteLimit);
    if (!rl.allowed) {
      const upgradePlan = nextPaidPlan(org?.subscription_tier);
      return NextResponse.json(
        {
          error: `You've used all ${noteLimit} AI note generations available in this hour.`,
          code: "NOTE_LIMIT_REACHED",
          limit: noteLimit,
          resetAt: new Date(rl.resetAt).toISOString(),
          upgradePlan,
          upgradeUrl: "/billing?reason=note-limit",
        },
